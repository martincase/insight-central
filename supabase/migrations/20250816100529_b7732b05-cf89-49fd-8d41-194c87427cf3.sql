-- Enhance the existing table and create new tables for comprehensive data banking

-- First, let's ensure the existing table has all needed indexes
CREATE INDEX IF NOT EXISTS idx_daily_account_snapshots_date ON "Daily Account Snapshots Table"(record_date);
CREATE INDEX IF NOT EXISTS idx_daily_account_snapshots_merchant_token ON "Daily Account Snapshots Table"(merchant_token);
CREATE INDEX IF NOT EXISTS idx_daily_account_snapshots_date_merchant ON "Daily Account Snapshots Table"(record_date, merchant_token);
...
ALTER TABLE "Daily Account Snapshots Table" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Allow all access to daily_account_snapshots" ON "Daily Account Snapshots Table" FOR ALL USING (true);
